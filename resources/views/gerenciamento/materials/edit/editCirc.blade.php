                    ...
                    <div class="panel panel-{{Auth::user()->css}}">
                        ...
                        <div class="panel panel-default">
                            <div class="panel-heading">                                
                                <h3 class="panel-title">Editar Circular Nº <strong>{{$circular->number}}/{{$circular->year}}</strong> - <strong>{{$circular['name']}}</strong></h3>
                                <ul class="panel-controls">
                                    <li><a href="#" class="panel-collapse"><span class="fa fa-angle-down"></span></a></li>
                                </ul>                                
                            </div>
                            <div class="panel-body">
                                <form enctype="multipart/form-data" method="POST" action="{{ route('saveFileCirc',['user' => Auth::user()->id] ) }}">
                                    @csrf
                                    <input type="hidden" name="id" value="{{$circular->id}}">
                                    <input type="hidden" name="ilha" value="{{$circular->ilha_id}}">
