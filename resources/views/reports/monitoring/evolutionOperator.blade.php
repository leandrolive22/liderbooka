...
<div class="page-container">
    ...
    <div class="page-content">
        ...
        <ul class="breadcrumb">
            ...
            <li><a href="#">Evolução Por Operador</a></li>
                ...
                <div class="col-md-12">
                                ...
                                <div class="panel panel-default tabs">
                                    <ul class="nav nav-tabs" role="tablist">
                                        <li class="active"><a href="#tab-first" role="tab" data-toggle="tab">Buscar Monitoria Com Evolução Por Operador</a></li>
                                    </ul>
                                    {{-- <li id="tab-fisrt"> --}}
                                        <form id="formSearch" @if(in_array(Auth::user()->cargo_id,[5])))action="{{ route('PostFeedbacksEvolutionOperatorPost', ['id'=>Auth::id()]) }}" @endif method="POST">
                                        @csrf
                                            <div class="panel-body">
                                                <div class="form-group">
                                                    <label  for="supers">Tipo Operador</label>
                                                    <label class="check">
                                                        <input type="radio" value="Novo" name="operador_novo" onchange="tpOperador(this)"/>
                                                        Novo
                                                    </label>
                                                    <label class="check">
                                                        <input type="radio" value="Maturado" name="operador_novo" onchange="tpOperador(this)"/>
                                                        Maturado
                                                    </label>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select data-live-search="true" name="supers" id="supers" onchange="$('#formSearch').prop('action','{{asset('monitoring/PostFeedbacksEvolutionOperator')}}'+'/'+$(this).val());" placeholder="Selecione o supervisor" class="form-control">
                                                        <option value="0">Selecione um Operador</option>
                                                        @forelse ($supers as $item)
                                                            <option value="{{$item->id}}" id="{{$item->operador_novo}}">{{$item->name}}</option>
                                                        @empty
                                        ...
                                        <div class="panel-footer">
                                            <button class="btn btn-primary pull-right" type="button" id="btnSearch">Buscar<span class="fa fa-search fa-right"></span></button>
                                        </div>
                                    </form>
                                   </div>
                               </div>
                            </form>
                {{-- Grafico Por Data / Operador --}}
                            ...
                            {{-- START DEFAULT DATATABLE --}}
                                 <div class="panel panel-default">
                                    ...
                                    <h3 class="panel-title">Média de Operadores Por Período</h3>
                                        <button id="btnExport" class="btn btn-outline-success pull-right">
                                          <span class="fa fa-table"></span>
                                          <span id="spanExport">Exportar</span>
                                        </button>
                                    </div>
                                </div>
                                <div class="panel-body">
                                    <table class="table table-bordered" id="exportTable">
                                        ...
                                        <tbody style="overflow-y: auto; max-heigth: 500px;">
                                           @forelse($result as $results)
                                                <tr>
                                                    <td>{{$results->nameOp}}</td>
                                                    <td>{{$results->nameSup}}</td>
                                                    <td>{{$results->media}}%</td>
                                                    <td>{{$results->conf}}</td>
                                                    <td>{{$results->nConf}}</td>
                                                    <td>{{$results->quartil}}</td>
                                                    <td>{{$results->date}}</td>
                                                    <td>{{$results->operador_novo}}</td>
                                                </tr>
                                            @empty
                                              <tr>
...
@section('Javascript')
    ...
    <script lang="javascript">
        ...
        */
        ...
        function tpOperador(element) {
            value = element.value
            if(value === "Novo") {
                $("option#Novo").show()
                $("option#Maturado").hide()
            } else if(value === "Maturado"){
                $("option#Novo").hide()
                $("option#Maturado").show()
            } else {
                $("option#Novo").show()
                $("option#Maturado").show()
            }
