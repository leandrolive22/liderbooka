...
class Calculadoras extends Controller
{
    ...
    public function CpfPending(Request $cpf) {
        ...
        // $log = new Logs();
        // $log->log('SEARCH_CPF', $cpf, $request->fullUrl(),Auth::id(),0);

        $base = DB::select('SELECT desconto_avista, desconto_avista2a4x, desconto_avista5a12x, desconto_avista13a24x, taxa_avista FROM book_materiais.base_dados WHERE cpf = ?', [$cpf]);
        
        return ($base);

    }

    // base Consulta - Saldo Remanescente Start
    public function indexBase() {
        $select = DB::table("book_materiais.base_consulta_saldo_remanescente")
                    ->select('*')
                    ->paginate(15);
        
        return  view('calculadoras.saldoRemanescente', compact('select'));
    }
    ...
    public function searchContract(Request $request)
    {
        ...
        $log = new Logs();
        $log->log('SEARCH_CONTRACT', $contrato, $request->fullUrl(), $user, $ilha);

        $select = DB::table("book_materiais.base_consulta_saldo_remanescente")
                    ->select('*')
                    ->where("Nr_Contrato_Origem","=",$contrato)
                    ->limit(1)
                    ->get();
        if(count($select) === 0) {
            return [
                "0" => [
                    'Escritorio' => 'Não Encontrado',
                    'Tel_Escritorio_1' => 'Não Encontrado',
                    'Tel_Escritorio_2' => 'Não Encontrado',
                    'Endereco_Escritorio' => 'Não Encontrado',
                    'Cidade_Escritorio' => 'Não Encontrado',
                    'UF_Escritorio' => 'Não Encontrado'
                ]
            ];
        }
